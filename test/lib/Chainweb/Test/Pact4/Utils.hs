...
mkCapability mn cap args = SigCapability (QualifiedName mn cap Pact.Types.Info.noInfo) args
...
--
buildTextCmd :: Version.HasVersion => Text -> CmdBuilder -> IO (Command Text)
buildTextCmd nonce = fmap (fmap T.decodeUtf8) . buildRawCmd nonce

-- | Build a raw bytestring command
--
buildRawCmd :: (MonadThrow m, MonadIO m) => Version.HasVersion => Text -> CmdBuilder -> m (Command ByteString)
buildRawCmd nonce (set cbNonce nonce -> CmdBuilder{..}) = do
    kps <- liftIO $ traverse mkDynKeyPairs _cbSigners
    cmd <- liftIO $ mkCommandWithDynKeys kps _cbVerifiers pm _cbNonce (Just nid) _cbRPC
    pure cmd
  where
    nid = P.NetworkId (sshow Version.implicitVersion)
...
--
someBlockHeader :: Version.HasVersion => BlockHeight -> BlockHeader
someBlockHeader 0 = genesisBlockHeader (unsafeChainId 0)
someBlockHeader h = (!! (int h - 1))
    $ testBlockHeaders
    $ Parent
    $ genesisBlockHeader (unsafeChainId 0)
