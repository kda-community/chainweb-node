...
buyGasShouldTakeGasTokensFromTheTransactionSender rdb = withVersion v $ readFromAfterGenesis rdb $ \blockEnv blockHandle ->
    flip evalStateT blockHandle $ doChainwebPactDbTransaction (blockEnv ^. psBlockDbEnv) Nothing $ \pactDb _spv -> do
        ...
        logger <- testLogger
        _ <- buyGas logger gasEnv pactDb noMiner (_psBlockCtx blockEnv) (view payloadObj <$> cmd)
...
buyGasFailures rdb = withVersion v $ readFromAfterGenesis rdb $ \blockEnv blockHandle -> do
    flip evalStateT blockHandle $ doChainwebPactDbTransaction (blockEnv ^. psBlockDbEnv) Nothing $ \pactDb _spv -> do
        ...
        do
            ...
            gasEnv <- mkTableGasEnv (MilliGasLimit mempty) GasLogsEnabled
            logger <- testLogger
            buyGas logger gasEnv pactDb noMiner (_psBlockCtx blockEnv) (view payloadObj <$> cmd)
...
redeemGasShouldGiveGasTokensToTheTransactionSenderAndMiner rdb =
    withVersion v $ readFromAfterGenesis rdb $ \blockEnv blockHandle -> do
        flip evalStateT blockHandle $ doChainwebPactDbTransaction (blockEnv ^. psBlockDbEnv) Nothing $ \pactDb _spv -> do
            ...
            logger <- testLogger
            redeemGas logger pactDb noMiner (_psBlockCtx blockEnv) (Gas 3) Nothing (view payloadObj <$> cmd)
...
redeemGasFailure rdb = withVersion v $ readFromAfterGenesis rdb $ \blockEnv blockHandle -> do
    flip evalStateT blockHandle $ doChainwebPactDbTransaction (blockEnv ^. psBlockDbEnv) Nothing $ \pactDb _spv -> do
        ...
        cmd <- buildCwCmd
            ...
            $ defaultCmd cid
        logger <- testLogger
        applyCmd logger Nothing pactDb miner (_psBlockCtx blockEnv) (TxBlockIdx 0) noSPVSupport (Gas 1) (view payloadObj <$> cmd)
            >>= P.match _Left
            ? P.match (_RedeemGasError . _RedeemGasPactError)
...
purchaseGasTxTooBig rdb = withVersion v $ readFromAfterGenesis rdb $ \blockEnv blockHandle -> do
    flip evalStateT blockHandle $ doChainwebPactDbTransaction (blockEnv ^. psBlockDbEnv) Nothing $ \pactDb _spv -> do
        cmd <- buildCwCmd
            ...
            $ defaultCmd cid
        logger <- testLogger
        applyCmd logger Nothing pactDb noMiner (_psBlockCtx blockEnv) (TxBlockIdx 0) noSPVSupport (Gas 2) (view payloadObj <$> cmd)
            >>= P.match _Left
...
runPayloadShouldReturnEvalResultRelatedToTheInputCommand rdb = withVersion v $ readFromAfterGenesis rdb $ \blockEnv blockHandle -> do
    flip evalStateT blockHandle $ doChainwebPactDbTransaction (blockEnv ^. psBlockDbEnv) Nothing $ \pactDb _spv -> do
        ...
        payloadResult <- runExceptT $
            runReaderT
                (runTransactionM
                    (runPayload Transactional Set.empty pactDb noSPVSupport [] managedNamespacePolicy gasEnv (_psBlockCtx blockEnv) (TxBlockIdx 0) (view payloadObj <$> cmd)))
...
applyLocalSpec rdb = withVersion v $ readFromAfterGenesis rdb $ \blockEnv blockHandle -> do
    flip evalStateT blockHandle $ doChainwebPactDbTransaction (blockEnv ^. psBlockDbEnv) Nothing $ \pactDb _spv -> do
        ...
        logger <- testLogger
        applyLocal logger Nothing pactDb (_psBlockCtx blockEnv) noSPVSupport (view payloadObj <$> cmd)
            >>= P.checkAll
...
applyCmdSpec rdb = withVersion v $ readFromAfterGenesis rdb $ \blockEnv blockHandle -> do
    bh <- flip execStateT blockHandle $ doChainwebPactDbTransaction (blockEnv ^. psBlockDbEnv) Nothing $ \pactDb _spv -> do
        startSender00Bal <- readBal pactDb "sender00"
...
applyCmdCoinTransfer rdb = withVersion v $ readFromAfterGenesis rdb $ \blockEnv blockHandle -> do
    flip evalStateT blockHandle $ doChainwebPactDbTransaction (blockEnv ^. psBlockDbEnv) Nothing $ \pactDb _spv -> do
        ...
        logger <- testLogger
        e <- applyCmd logger Nothing pactDb noMiner (_psBlockCtx blockEnv) (TxBlockIdx 0) noSPVSupport (Gas 1) (view payloadObj <$> cmd)
        e & P.match _Right
            ? P.checkAll
...
testWritesFromFailedTxDontMakeItIn rdb = withVersion v $ readFromAfterGenesis rdb $ \blockEnv blockHandle -> do
    bh <- flip execStateT blockHandle $ doChainwebPactDbTransaction (blockEnv ^. psBlockDbEnv) Nothing $ \pactDb _spv -> do
        ...
        logger <- testLogger
        applyCmd logger Nothing pactDb noMiner (_psBlockCtx blockEnv) (TxBlockIdx 0) noSPVSupport (Gas 1) (view payloadObj <$> moduleDeploy)
            >>= P.match _Right
            ? P.fun _crResult
