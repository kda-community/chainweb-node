...
module Chainweb.Test.Pact.SignedListPluginTests (tests) where
...
signedListTestTemplate baseRdb SignedListTest{..} = withVersion v $ testCaseSteps sltName $ \step -> runResourceT $ do
  ...
  liftIO $ do
    ...
    poll fx chain0 [cmdToRequestKey usePlugin] >>= \result -> do
  ...
  where
    deployContract :: HasVersion => Fixture -> Step -> IO ()
    deployContract fx step = do
      ...
      advanceAllChains_ fx
      poll fx chain0 [cmdToRequestKey deploy] >>= P.list [P.match _Just successfulTx]
